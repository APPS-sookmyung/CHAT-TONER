...
async def generate_final_integrated_text(
    ...
    try:
        ...
        draft_prompt = (
            ...
            "- 출력은 Markdown 형식으로 작성:\n"
            "  • 맨 위에 '## 최종 글' 제목 추가\n"
            "  • 본문은 문단/불릿을 적절히 사용하여 가독성 있게 구성\n"
            "  • 불필요한 머리말/설명 금지, 본문만 출력\n\n"
        ...
        polish_prompt = (
            ...
            "- 출력은 Markdown 형식 유지: '## 최종 글' + 문단/불릿 구성, 본문만 제공\n\n"
            f"초안:\n{draft_text}\n\n"
            "최종 글:"
        )
        ...
        # 강제 반영: AFTER 문구 반영이 거의 없으면 한 번 더 LLM 호출로 강제 적용
        def _hits(text: str, items: list[dict]) -> int:
            cnt = 0
            for it in items:
                aft = (it.get('after') or '').strip()
                if aft and aft in text:
                    cnt += 1
            return cnt

        applied_hits = _hits(final_text, all_items)
        required_hits = max(1, len(all_items) // 2)
        if applied_hits < required_hits:
            enforce_list = "\n".join([f"- {it['after']}" for it in all_items if (it.get('after') or '').strip()])
            force_prompt = (
                "아래 원문을 기반으로 반드시 나열된 문구/표현(AFTER)을 반영하여 최종 글을 작성하세요.\n"
                "- 나열된 AFTER 문구가 본문에 실제로 등장해야 합니다\n"
                "- 의미 오류 없이 자연스럽게 통합\n"
                "- 출력은 Markdown 형식: 상단 '## 최종 글' 제목 + 문단/불릿 구성, 본문만 제공\n\n"
                f"원문:\n{request.original_text}\n\n"
                f"반영 필수 문구(AFTER):\n{enforce_list}\n\n"
                "최종 글:"
            )
            forced = await oai.generate_text(force_prompt, temperature=0.2, max_tokens=800)
