...
class ProfileGeneratorService:
    ...
    async def create_profile_from_documents(self, company_id: str, documents: list[str]) -> str:
        """Generate company onboarding characteristics summary from documents."""
        
        document_extracts = "\n".join([f"- {doc[:200]}..." for doc in documents])

        prompt = f"""
        다음은 '{company_id}' 회사의 내부 문서 일부입니다.

        {document_extracts}

        위 문서들을 바탕으로, 이 회사의 신규 입사자를 위한 온보딩 특성을 2-3개의 핵심 문장으로 요약해주세요.
        - 초점: 회사의 문화, 소통 방식, 핵심 가치
        - 톤: 친근하고 명료하게
        """
        ...
        try:
            profile_text = await self.openai_service.generate_text(prompt)
            logger.info(f"문서 기반 기업 프로필 LLM 생성 완료: {company_id}")
            return profile_text.strip()
        except Exception as e:
            logger.error(f"문서 기반 기업 프로필 LLM 생성 실패: {e}")
            return "문서 내용을 기반으로 한 회사 온보딩 특성 요약 생성에 실패했습니다."
