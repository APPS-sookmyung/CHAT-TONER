...
export default function TransformStylePage() {
      ...
      try {
        ...
        const result = await api.convertStyle({
          ...
          user_profile: userProfile
            ? {
                baseFormalityLevel: clampToScale(userProfile.baseFormalityLevel),
                baseFriendlinessLevel: clampToScale(
                  userProfile.baseFriendlinessLevel
                ),
                baseEmotionLevel: clampToScale(userProfile.baseEmotionLevel),
                baseDirectnessLevel: clampToScale(userProfile.baseDirectnessLevel),
              }
            : {
        ...
        // Pick converted text based on selected style for initial display
        let convertedText = text;
        if (allConvertedTexts && typeof allConvertedTexts === "object") {
          if (style === "directness") {
            convertedText = (allConvertedTexts as any).direct || text;
          } else if (style === "softness") {
            convertedText = (allConvertedTexts as any).gentle || text;
          } else if (style === "politeness") {
            convertedText = (allConvertedTexts as any).neutral || text;
          } else {
            convertedText =
              (allConvertedTexts as any).direct ||
              (allConvertedTexts as any).gentle ||
              (allConvertedTexts as any).neutral ||
              text;
          }
        }
        ...
        return {
          ...
          converted_texts: allConvertedTexts as any,
          suggestions: [],
        };
      } catch (error) {
  ...
  const outputValue = useMemo(() => {
    ...
    if (analysis.converted_texts) {
      ...
      if (selectedStyle === "softness" && analysis.converted_texts.gentle) {
        return analysis.converted_texts.gentle;
      }
      if (selectedStyle === "politeness" && analysis.converted_texts.neutral) {
        ...
        {analysis && (
          <div className="flex items-center gap-2 self-start ml-[10%]">
            ...
            <span className="text-[10px] text-gray-400">
              Filtering: Emojis, Slang, Casual Tone
            </span>
          </div>
        )}
